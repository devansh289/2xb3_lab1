...
def merge_sort(L):
    if len(L) > 1:
        mid = len(L)//2
        left_division = L[:mid]
        right_division = L[mid:]

        merge_sort(left_division)
        merge_sort(right_division)
        ...
        while i < len(left_division) and j < len(right_division):
            if left_division[i] < right_division[j]:
                L[k] = left_division[i]
                i += 1
            else:
                L[k] = right_division[j]
                j += 1
            k += 1

        # Copying leftover array
        while i < len(left_division):
            L[k] = left_division[i]
            i += 1
            k += 1
        while j < len(right_division):
            L[k] = right_division[j]
            j += 1
            k += 1
